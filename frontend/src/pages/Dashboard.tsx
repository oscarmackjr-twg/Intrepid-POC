...
export default function Dashboard() {
  ...
  const triggerRun = async (tdayYYYYMMDD: string | null) => {
    try {
      ...
      // Run is created immediately (202); refresh list so new run appears with PENDING then RUNNING
      fetchRuns()
    } catch (error: any) {
    ...
    <div className="px-4 py-6 sm:px-0">
      ...
      {user?.role === 'admin' && (
        <div className="bg-white shadow rounded-lg p-4 mb-6 border border-gray-200">
          <h2 className="text-lg font-medium text-gray-900 mb-2">Admin: run management</h2>
          <p className="text-sm text-gray-500 mb-3">
            Cancel all runs stuck in &quot;running&quot; or clear all job history so the dashboard starts fresh.
          </p>
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              disabled={adminActionLoading || !hasRunningRun}
              onClick={async () => {
                if (!confirm('Cancel all runs that are currently in &quot;running&quot; state?')) return
                setAdminActionLoading(true)
                try {
                  const res = await axios.post('/api/admin/runs/cancel-all-running')
                  alert(res.data?.message ?? `Cancelled ${res.data?.cancelled ?? 0} run(s).`)
                  fetchRuns()
                } catch (e: any) {
                  alert(e.response?.data?.detail ?? e.message ?? 'Failed to cancel runs')
                } finally {
            ...
            >
              {adminActionLoading ? '…' : 'Cancel all running runs'}
            </button>
            <button
              type="button"
              disabled={adminActionLoading}
              onClick={async () => {
                if (!confirm('Permanently delete ALL run history (runs, exceptions, loan facts)? Dashboard will be empty.')) return
                setAdminActionLoading(true)
                try {
                  const res = await axios.post('/api/admin/runs/clear-history')
                  alert(res.data?.message ?? 'History cleared.')
                  fetchRuns()
                } catch (e: any) {
                  alert(e.response?.data?.detail ?? e.message ?? 'Failed to clear history')
                } finally {
                  setAdminActionLoading(false)
                }
              }}
            ...
            >
              {adminActionLoading ? '…' : 'Clear all run history'}
            </button>
          </div>
        </div>
      )}
