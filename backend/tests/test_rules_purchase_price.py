...
class TestGetPurchasePriceExceptions:
    ...
    def test_exception_generation(self):
        ...
        get_purchase_price_exceptions returns dicts with key 'seller_loan_number'
        (not 'loan_number'). The message does not include the loan number —
        it contains Lender Price and Modeled values only.
        """
        df = pd.DataFrame({
        ...
        # The loan identifier is in 'seller_loan_number', not in 'message'
        assert exceptions[0]['seller_loan_number'] == 'SFC_1001'
        assert exceptions[1]['seller_loan_number'] == 'SFC_1002'
