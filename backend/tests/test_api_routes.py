...
class TestAuthentication:
    ...
    def test_login_success(self, client, sample_admin_user):
        ...
        response = client.post("/api/auth/login", data={"username": "admin", "password": "testpass"})
        assert response.status_code == 200
    ...
    def test_login_invalid_credentials(self, client):
        ...
        response = client.post("/api/auth/login", data={"username": "invalid", "password": "wrong"})
...
class TestRunsEndpoint:
    ...
    def test_list_runs(self, client, auth_headers_admin, test_db_session):
        """Test listing runs."""
        # Create sample run
        run = PipelineRun(run_id="test_run_123", status=RunStatus.COMPLETED, total_loans=10, total_balance=100000.0)
    ...
    def test_get_run(self, client, auth_headers_admin, test_db_session):
        ...
        run = PipelineRun(run_id="test_run_456", status=RunStatus.COMPLETED)
        test_db_session.add(run)
        test_db_session.commit()

        response = client.get("/api/runs/test_run_456", headers=auth_headers_admin)
        assert response.status_code == 200
    ...
    def test_sales_team_isolation(self, client, auth_headers_sales, test_db_session, sample_sales_team):
        ...
        other_run = PipelineRun(run_id="other_run", status=RunStatus.COMPLETED, sales_team_id=other_team.id)
        test_db_session.add(other_run)

        # Create run for user's sales team
        user_run = PipelineRun(run_id="user_run", status=RunStatus.COMPLETED, sales_team_id=sample_sales_team.id)
...
class TestExceptionsEndpoint:
    ...
    def test_get_exceptions_with_filters(self, client, auth_headers_admin):
        ...
        response = client.get(
            "/api/exceptions",
            params={"exception_type": "purchase_price", "severity": "error"},
