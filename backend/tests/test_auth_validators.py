...
class TestValidateSalesTeamAssignment:
    ...
    def test_sales_team_with_assignment(self, test_db_session, sample_sales_team):
        ...
        validate_sales_team_assignment(UserRole.SALES_TEAM, sample_sales_team.id, test_db_session)
        # Should not raise

    def test_sales_team_without_assignment(self, test_db_session):
        """Test SALES_TEAM user without sales_team_id."""
        with pytest.raises(HTTPException) as exc_info:
            validate_sales_team_assignment(UserRole.SALES_TEAM, None, test_db_session)
    ...
    def test_admin_without_assignment(self, test_db_session):
        ...
        validate_sales_team_assignment(UserRole.ADMIN, None, test_db_session)
        # Should not raise

    def test_analyst_without_assignment(self, test_db_session):
        """Test ANALYST user without sales_team_id (should be OK)."""
        validate_sales_team_assignment(UserRole.ANALYST, None, test_db_session)
        # Should not raise
...
class TestValidateSalesTeamAccess:
    ...
    def test_sales_team_no_assignment(self, test_db_session):
        ...
        user = User(
            email="test@test.com", username="test", hashed_password="hash", role=UserRole.SALES_TEAM, sales_team_id=None
        )
...
class TestValidateUserUpdate:
    ...
    def test_admin_can_update(self, test_db_session, sample_admin_user, sample_sales_team):
        ...
        target_user = User(email="target@test.com", username="target", hashed_password="hash", role=UserRole.ANALYST)
        test_db_session.add(target_user)
        test_db_session.commit()

        validate_user_update(
            target_user.id, UserRole.SALES_TEAM, sample_sales_team.id, sample_admin_user, test_db_session
        )
        # Should not raise
    ...
    def test_non_admin_cannot_update(self, test_db_session, sample_sales_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            validate_user_update(target_user.id, None, None, sample_sales_user, test_db_session)
        assert exc_info.value.status_code == 403

    def test_cannot_change_own_role(self, test_db_session, sample_admin_user):
        """Test user cannot change own role."""
        with pytest.raises(HTTPException) as exc_info:
            validate_user_update(sample_admin_user.id, UserRole.SALES_TEAM, None, sample_admin_user, test_db_session)
        assert exc_info.value.status_code == 403
