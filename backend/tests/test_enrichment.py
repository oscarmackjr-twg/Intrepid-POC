...
class TestTagLoansByGroup:
    ...
    def test_sfy_tagging(self):
        """Test SFY tagging."""
        df = pd.DataFrame({"Loan Group": ["FX3_GROUP", "FX1_GROUP", "FX3_SOMETHING"]})
    ...
    def test_prime_tagging(self):
        """Test PRIME tagging."""
        df = pd.DataFrame({"Loan Group": ["PRIME_GROUP", "OTHER_GROUP", "STANDARD"]})
    ...
    def test_mixed_tagging(self):
        ...
        df = pd.DataFrame({"Loan Group": ["FX3_GROUP", "PRIME_GROUP", "FX1_GROUP"]})
...
class TestAddSellerLoanNumber:
    ...
    def test_add_seller_loan_number(self):
        ...
        df = pd.DataFrame({"Account Number": [1001, 1002, 1003]})
...
class TestMarkRepurchasedLoans:
    ...
    def test_repurchased_detection(self):
        ...
        df = pd.DataFrame({"Status Codes": ["", "REPURCHASE", "ACTIVE;REPURCHASE", "ACTIVE"]})
...
class TestEnrichBuyDf:
    ...
    def test_basic_enrichment(self, sample_buy_df, sample_loans_types_df):
        ...
        result = enrich_buy_df(sample_buy_df, sample_loans_types_df, pdate="2024-11-18", irr_target=8.05)
