...
class TestNormalizeSfyDf:
    ...
    def test_header_row_skipping(self):
        ...
        normalize_sfy_df uses df.iloc[4:] to skip 4 junk rows, then promotes
        the first remaining row as column headers, then returns df[1:].
        A 6-row input (4 junk + 1 header row + 1 data row) leaves exactly 1
        data row after the skip+header-promotion.
        """
        # 4 junk rows + 1 header row + 1 data row = 6 rows total
        df = pd.DataFrame({
            'A': ['', '', '', '', 'Header1', 'Data1'],
            'B': ['', '', '', '', 'Header2', 'Data2'],
        })
        ...
        # Should have skipped first 4 rows and header row, leaving 1 data row
        assert len(result) == 1
    ...
    def test_tu144_column_standardization(self):
        ...
        Requires 6-row input so that data survives the 4-row skip + header-promotion.
        """
        # 4 junk rows + 1 header row (contains TU_144) + 1 data row
        df = pd.DataFrame({
            'A': ['', '', '', '', 'TU_144', '1'],
            'B': ['', '', '', '', 'Other', 'Data'],
        })
        ...
        # Should standardize TU_144 to TU144
...
class TestNormalizePrimeDf:
    ...
    def test_header_row_skipping(self):
        ...
        normalize_prime_df uses df.iloc[4:] to skip 4 junk rows, then promotes
        the first remaining row as column headers, then returns df[1:].
        A 6-row input (4 junk + 1 header row + 1 data row) leaves exactly 1
        data row after the skip+header-promotion.
        """
