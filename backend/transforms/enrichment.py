...
def tag_loans_by_group(loans_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = loans_df.copy()
    df["tagging"] = df["Loan Group"].apply(lambda x: "SFY" if "FX3" in str(x) or "FX1" in str(x) else "PRIME")
    return df
...
def mark_repurchased_loans(loans_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Status Codes" in df.columns:
        # Avoid chained assignment / inplace ops that can raise ChainedAssignmentError in pandas 2
...
def enrich_buy_df(buy_df: pd.DataFrame, df_loans_types: pd.DataFrame, pdate: str, irr_target: float) -> pd.DataFrame:
    ...
    if "loan program" in df.columns and "Platform" in df.columns:
        df = df.merge(df_loans_types, on=["loan program", "Platform"], how="left")
