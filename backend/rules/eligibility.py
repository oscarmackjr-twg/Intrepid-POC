...
def check_eligibility_prime(final_df_all: pd.DataFrame) -> Dict[str, Any]:
    ...
    prime_df = final_df_all[(final_df_all["platform"] == "prime") & (final_df_all["Excess_Asset"] == False)].copy()
    ...
    check_c = (
        prime_df[(prime_df["Term"] > 144) & (prime_df["type"] == "standard") & (prime_df["FICO Borrower"] >= 700)][
            "Orig. Balance"
        ].sum()
        / total_balance
    )
    ...
    check_h2 = (
        prime_df[(prime_df["Lender Price(%)"] > 100) & (prime_df["Lender Price(%)"] <= 103)]["Orig. Balance"].sum()
        / total_balance
    )
    ...
    try:
        state_distribution = (
            pd.pivot_table(prime_df, values="Orig. Balance", aggfunc="sum", columns="Property State") / total_balance
        )
        results["check_j_state_dist"] = {
            "value": state_distribution.T.sort_values("Orig. Balance", ascending=False).to_dict()
    ...
    if len(non_repurchase_prime) > 0:
        ...
        _new_prog_mask = (
            non_repurchase_prime["new_programs"] == True
            if "new_programs" in non_repurchase_prime.columns
            else pd.Series([False] * len(non_repurchase_prime), index=non_repurchase_prime.index)
        )
        check_s1 = (
            non_repurchase_prime[_new_prog_mask]["Orig. Balance"].sum() / non_repurchase_total
            if non_repurchase_total > 0
            else 0
        )
...
def check_eligibility_sfy(final_df_all: pd.DataFrame, buy_df: pd.DataFrame = None) -> Dict[str, Any]:
    ...
    sfy_df = final_df_all[(final_df_all["platform"] == "sfy") & (final_df_all["Excess_Asset"] == False)].copy()
    for _col, _default in [
        ("promo_term", 0),
        ("Term", 0),
        ("APR", 0),
    ...
    check_a2 = sfy_df[(sfy_df["type"] == "hybrid") & (sfy_df["APR"] < 7.0)]["Orig. Balance"].sum() / total_balance
    ...
    check_b2 = sfy_df[(sfy_df["type"] == "ninp") & (sfy_df["promo_term"] > 6)]["Orig. Balance"].sum() / total_balance
    check_b3 = sfy_df[(sfy_df["type"] == "ninp") & (sfy_df["promo_term"] > 12)]["Orig. Balance"].sum() / total_balance
    check_b4 = sfy_df[(sfy_df["type"] == "ninp") & (sfy_df["Term"] > 84)]["Orig. Balance"].sum() / total_balance
    ...
    check_d3 = sfy_df[(sfy_df["type"] == "wpdi") & (sfy_df["promo_term"] >= 12)]["Orig. Balance"].sum() / total_balance
    check_d4 = (
        sfy_df[(sfy_df["type"].isin(["wpdi", "wpdi_bd"])) & (sfy_df["promo_term"] >= 12)]["Orig. Balance"].sum()
        / total_balance
    )
    ...
    check_e1 = sfy_df[(sfy_df["type"] == "standard") & (sfy_df["Term"] > 120)]["Orig. Balance"].sum() / total_balance
    check_e2 = (
        sfy_df[(sfy_df["type"].isin(["standard", "standard_bd"])) & (sfy_df["Term"] > 120)]["Orig. Balance"].sum()
        / total_balance
    )
    check_e3 = sfy_df[(sfy_df["type"] == "standard") & (sfy_df["Term"] > 144)]["Orig. Balance"].sum() / total_balance
    check_e4 = (
        sfy_df[(sfy_df["type"].isin(["standard", "standard_bd"])) & (sfy_df["Term"] > 144)]["Orig. Balance"].sum()
        / total_balance
    )
    ...
    check_f2 = (
        sfy_df[(sfy_df["Lender Price(%)"] > 100) & (sfy_df["Lender Price(%)"] <= 103)]["Orig. Balance"].sum()
        / total_balance
    )
    check_f3 = (
        sfy_df[
    ...
    try:
        state_distribution = (
            pd.pivot_table(sfy_df, values="Orig. Balance", aggfunc="sum", columns="Property State") / total_balance
        )
    ...
    if "Purchase Price" in final_df_all.columns and final_df_all["Purchase Price"].sum() > 0:
        check_l3 = (
            final_df_all[final_df_all["type"].isin(["standard_bd", "wpdi_bd"])]["Purchase Price"].sum()
            / final_df_all["Purchase Price"].sum()
        )
        check_l4 = (
            final_df_all[
                (final_df_all["type"].isin(["standard_bd", "wpdi_bd"])) & (final_df_all["Excess_Asset"] != True)
            ]["Purchase Price"].sum()
            / final_df_all["Purchase Price"].sum()
        )
    else:
    ...
    _new_prog_sfy = (
        final_df_all["new_programs"] == True
        if "new_programs" in final_df_all.columns
        else pd.Series([False] * len(final_df_all), index=final_df_all.index)
    )
    check_s1 = (
        final_df_all[_new_prog_sfy & (final_df_all["Repurchase"] == False) & (final_df_all["platform"] == "sfy")][
            "Orig. Balance"
        ].sum()
        / final_df_all[(final_df_all["Repurchase"] == False) & (final_df_all["platform"] == "sfy")][
            "Orig. Balance"
        ].sum()
        if len(final_df_all[(final_df_all["Repurchase"] == False) & (final_df_all["platform"] == "sfy")]) > 0
        else 0
    )
    results["check_s1"] = {"value": check_s1, "pass": check_s1 < 0.02}
