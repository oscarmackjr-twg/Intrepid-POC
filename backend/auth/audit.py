...
def log_user_action(
    ...
    logger.info(
        "audit: action=%s user=%s outcome=%s source_ip=%s resource=%s",
        action,
...
def log_data_access(
    user: User, resource_type: str, resource_id: Optional[str] = None, sales_team_id: Optional[int] = None
):
    """
...
def log_authorization_failure(user: User, action: str, reason: str, resource_id: Optional[str] = None):
    """
