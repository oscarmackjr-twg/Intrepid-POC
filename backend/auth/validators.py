...
def validate_sales_team_assignment(role: UserRole, sales_team_id: int | None, db: Session) -> None:
    """
    ...
    if role == UserRole.SALES_TEAM:
        if sales_team_id is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Sales team users must be assigned to a sales team"
            )

        # Verify sales team exists and is active
        sales_team = db.query(SalesTeam).filter(SalesTeam.id == sales_team_id, SalesTeam.is_active == True).first()
...
def validate_sales_team_access(user: User, target_sales_team_id: int | None) -> None:
    """
    ...
    if user.role == UserRole.SALES_TEAM:
        ...
        if target_sales_team_id is not None and user.sales_team_id != target_sales_team_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Access denied: Cannot access other sales team's data"
            )


def validate_user_update(
    user_id: int, role: UserRole | None, sales_team_id: int | None, current_user: User, db: Session
) -> None:
    """
...
def get_user_sales_team_id(user: User) -> int | None:
    ...
    if user.role == UserRole.SALES_TEAM:
        if user.sales_team_id is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Sales team user must have sales_team_id assigned"
            )
