...
def send_notification(
    event_type: str, sales_team_id: Optional[int], message: str, details: Optional[Dict[str, Any]] = None
):
    """
...
def notify_run_started(sales_team_id: Optional[int], run_id: str):
    ...
    send_notification("run_started", sales_team_id, f"Pipeline run started for {team_str}", {"run_id": run_id})
...
def notify_run_failed(sales_team_id: Optional[int], run_id: Optional[str], error: str):
    ...
    team_str = f"sales_team_{sales_team_id}" if sales_team_id else "general"
    send_notification(
        "run_failed", sales_team_id, f"Pipeline run failed for {team_str}", {"run_id": run_id, "error": error}
    )
