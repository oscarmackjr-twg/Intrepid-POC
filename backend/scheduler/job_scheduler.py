...
async def run_daily_pipeline(sales_team_id: Optional[int] = None):
    ...
    try:
        ...
        error_msg = str(e)
        logger.error(f"Scheduled pipeline run failed for sales_team_id={sales_team_id}: {error_msg}", exc_info=True)
...
def schedule_daily_runs():
    ...
    try:
        ...
        for team in sales_teams:
            ...
            logger.info(
                f"Scheduled daily pipeline for sales team: {team.name} (ID: {team.id}) at {settings.DAILY_RUN_TIME}"
            )
