...
async def get_config(current_user: User = Depends(get_current_user)):
    """Return app config for the UI (storage type, S3/local paths)."""
    out: Dict[str, Any] = {"storage_type": settings.STORAGE_TYPE}
    if settings.STORAGE_TYPE == "s3" and settings.S3_BUCKET_NAME:
        ...
        # Expose S3 prefixes as configured via S3_INPUT / S3_OUTPUT / S3_OUTPUT_SHARED
        out["s3_input"] = getattr(settings, "S3_INPUT", None) or getattr(settings, "S3_INPUTS_PREFIX", "input")
        out["s3_output"] = getattr(settings, "S3_OUTPUT", None) or "outputs"
        out["s3_output_shared"] = getattr(settings, "S3_OUTPUT_SHARED", None) or "output_share"
        if settings.S3_BASE_PREFIX:
            out["s3_base_prefix"] = settings.S3_BASE_PREFIX
    else:
        # Local storage: expose effective directories after DEV_* overrides
        out["local_input_dir"] = settings.INPUT_DIR
        out["local_output_dir"] = settings.OUTPUT_DIR
        out["local_output_share_dir"] = settings.OUTPUT_SHARE_DIR
    return out
...
def _run_pipeline_background(context: RunContext, run_data: RunCreate) -> None:
    """Run pipeline in a background thread. Uses existing run record (existing_run_id). On failure, sets run to FAILED."""
    temp_dir = None
    try:
        if settings.STORAGE_TYPE == "s3":
            logger.info("Pipeline run run_id=%s syncing S3 inputs", context.run_id)
        ...
        logger.info("Pipeline run run_id=%s was cancelled", context.run_id)
    except Exception as e:
        ...
        try:
            ...
            if run:
                ...
                run.errors = (run.errors or []) + [str(e)]
                db.commit()
        finally:
            db.close()
    finally:
...
async def create_pipeline_run(
    ...
    if prev_run and prev_run.run_id != context.run_id:
        try:
            ...
            logger.warning("Archive previous run failed (continuing): %s", e)

    # Create run record immediately (PENDING) so it appears in UI and logs
    run_weekday, run_weekday_name = _weekday_from_pdate(context.pdate)
    run_record = PipelineRun(
        run_id=context.run_id,
        status=RunStatus.PENDING,
        sales_team_id=context.sales_team_id,
        created_by_id=context.created_by_id,
        pdate=context.pdate,
        irr_target=context.irr_target,
    ...
    logger.info("Run record created run_id=%s status=PENDING", context.run_id)

    # Start pipeline in background; response returns immediately
...
# ---------- Admin run management (cancel all running, clear history) ----------
@router.post("/admin/runs/cancel-all-running")
async def admin_cancel_all_running(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_role([UserRole.ADMIN])),
):
    """Cancel every run that is in RUNNING state (admin only). Use to clear stuck/running runs so the dashboard is clear."""
    running = db.query(PipelineRun).filter(PipelineRun.status == RunStatus.RUNNING).all()
    count = 0
    for run in running:
        run.status = RunStatus.CANCELLED
        run.completed_at = datetime.utcnow()
        err_list = list(run.errors) if run.errors else []
        err_list.append("Cancelled by admin (cancel-all-running)")
        run.errors = err_list
        count += 1
    db.commit()
    logger.info("Admin cancel-all-running: cancelled %d runs (user_id=%s)", count, current_user.id)
    return {"cancelled": count, "message": f"Cancelled {count} running run(s)."}


@router.post("/admin/runs/clear-history")
async def admin_clear_run_history(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_role([UserRole.ADMIN])),
):
    """Delete all pipeline runs and their related loan exceptions and loan facts (admin only). Dashboard and job history will be empty."""
    # Delete in order: child tables first (foreign key to pipeline_runs.id)
    deleted_exceptions = db.query(LoanException).delete()
    deleted_facts = db.query(LoanFact).delete()
    deleted_runs = db.query(PipelineRun).delete()
    db.commit()
    logger.info(
    ...
    return {
        ...
        "message": "All run history cleared. Dashboard will show no runs.",
    }
