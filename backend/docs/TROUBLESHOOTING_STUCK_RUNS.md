...
### 4. Run the pipeline externally (full logs for troubleshooting)

When a run is stuck at a phase (e.g. "Running eligibility checks") and you need to see exactly where it fails or hangs, run the pipeline **from the command line**. All log output goes to stdout so you get full phase progress and any stack traces.

**From project root** (with `DATABASE_URL` and, for QA, `STORAGE_TYPE=s3` and AWS credentials set):

```bash
# Re-run using the same parameters as the stuck run (loads pdate, input path from DB).
# For QA (S3 inputs) use --sync-s3 so inputs are downloaded to a temp dir.
python backend/scripts/run_pipeline_cli.py --run-id run_886742a6378a_20260301_223021 --sync-s3

# Or run with explicit folder and pdate (local inputs)
python backend/scripts/run_pipeline_cli.py --folder path/to/inputs --pdate 2026-03-04

# QA only: sync S3 to temp and run with a given pdate
python backend/scripts/run_pipeline_cli.py --sync-s3 --pdate 2026-03-04
```

The CLI run creates a **new** run (new run_id) and does not change the stuck run. Use `fix_stuck_runs.py --run-id <id> --mark failed` to clear the stuck run in the UI after you finish troubleshooting.

**Where to run:**

- **Locally**: Point `DATABASE_URL` at QA RDS (if allowed) or a local DB with the same schema; use `--sync-s3` so inputs match QA.
- **On the server / ECS**: Run the same script inside a one-off ECS task or an interactive shell that has access to the same DB and S3 (e.g. `docker run ... python backend/scripts/run_pipeline_cli.py --run-id <id> --sync-s3`).

### 5. Prevent future stuck runs (optional)
