...
def find_file_by_pattern(
    directory: str, pattern: str, date_str: Optional[str] = None, required: bool = True
) -> Optional[Path]:
    """
    ...
    if len(matches) > 1:
        ...
        logger.warning(f"Multiple files match pattern '{pattern}'. Using most recent: {matches[0].name}")
...
def find_tape_loans_file(directory: str, yesterday: str, required: bool = True) -> Optional[Path]:
    ...
    return find_file_by_pattern(f"{directory}/files_required", pattern, required=required)
...
def find_sfy_file(directory: str, date_str: Optional[str] = None, required: bool = True) -> Optional[Path]:
    ...
    if date_str:
        result = find_file_by_pattern(
            files_dir, f"SFY_{date_str}_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx", required=False
        )
        if result:
            return result
    else:
    ...
    primary_pattern = (
        f"SFY_{date_str}_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx"
        if date_str
        else "SFY_*_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx"
    )
...
def find_prime_file(directory: str, date_str: Optional[str] = None, required: bool = True) -> Optional[Path]:
    ...
    if date_str:
        result = find_file_by_pattern(
            files_dir, f"PRIME_{date_str}_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx", required=False
        )
        if result:
            return result
    else:
        result = find_file_by_pattern(
            files_dir, "PRIME_*_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx", required=False
        )
        if result:
            return result
    # Off-cycle fallback: spacing-agnostic (filenames may have extra spaces before Off-cycle)
    ...
    # Re-run with required=True to surface a clean error
    primary_pattern = (
        f"PRIME_{date_str}_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx"
        if date_str
        else "PRIME_*_ExhibitAtoFormofSaleNotice - Pre-Funding.xlsx"
    )
    return find_file_by_pattern(files_dir, primary_pattern, required=required)
