...
from transforms.enrichment import tag_loans_by_group, add_seller_loan_number, mark_repurchased_loans, enrich_buy_df
...
class PipelineExecutor:
    ...
    def append_run_log(self, message: str) -> None:
        """Append a user-visible log line to the run record (shown in UI Standard Output)."""
        if not self.run_record:
            return
        try:
            logs = list(self.run_record.errors) if self.run_record.errors else []
        except TypeError:
            logs = []
        logs.append(message)
        self.run_record.errors = logs
        self.db.commit()
    ...
    def execute(self, folder: str, existing_run_id: Optional[str] = None) -> Dict[str, Any]:
        """Execute the full pipeline. If existing_run_id is set, adopt that run (already created by API) and set RUNNING."""
        run_id = self.context.run_id
        try:
            logger.info(
                "Pipeline execute starting run_id=%s folder=%s existing_run_id=%s", run_id, folder, existing_run_id
            )
            if existing_run_id:
                self.run_record = self.db.query(PipelineRun).filter(PipelineRun.run_id == existing_run_id).first()
                if not self.run_record:
            ...
            self.append_run_log("Loaded reference data (MASTER_SHEET, underwriting grids, CoMAP tables).")
            ...
            self.append_run_log("Loaded input files (Tape20Loans, SFY exhibit, PRIME exhibit).")
            ...
            self.append_run_log(f"Normalized and tagged {len(loans)} loans.")
            ...
            buy_df["loan program"] = buy_df.apply(
                lambda x: (
                    str(x["loan program"]) + "notes"
                    if x.get("Application Type") == "HD NOTE"
                    else str(x["loan program"])
                ),
                axis=1,
            )
            ...
            self.append_run_log("Purchase price checks completed.")
            ...
            for seller_loan_number, _prog, platform in loan_not_in_comap:
                exc_type = (
                    "comap_prime" if platform == "PRIME" else ("comap_sfy" if platform == "SFY" else "comap_notes")
                )
            ...
            self.append_run_log(f"CoMAP checks completed. Loans not in grid: {len(loan_not_in_comap)}.")
            ...
            self.append_run_log("Eligibility checks completed.")
            ...
            eligibility_report_path = export_eligibility_report(
                eligibility_prime,
                eligibility_sfy,
                f"{settings.OUTPUT_DIR}/{output_prefix}"
                if settings.STORAGE_TYPE == "local"
                else f"/tmp/{output_prefix}",
            )
            ...
            for name, key in [
                ("eligibility_checks.json", "eligibility_checks_json"),
                ("eligibility_checks_summary.xlsx", "eligibility_checks_summary"),
            ]:
                p = elig_dir / name
                if p.exists():
            ...
            self.append_run_log("Saved exceptions and loan facts to database.")
            ...
            eligibility_summary = {
                "prime": {
                    ...
                    "passed": sum(
                        1 for v in eligibility_prime.values() if isinstance(v, dict) and v.get("pass") == True
                    ),
                    "failed": sum(
                        1 for v in eligibility_prime.values() if isinstance(v, dict) and v.get("pass") == False
                    ),
                },
                "sfy": {
                    ...
                    "failed": sum(
                        1 for v in eligibility_sfy.values() if isinstance(v, dict) and v.get("pass") == False
                    ),
                },
            }
            ...
            logger.error("Pipeline execution failed run_id=%s last_phase=%s: %s", run_id, last_phase, e, exc_info=True)
            # Preserve any prior log lines and append the error for UI visibility
            logs: List[str] = []
            if self.run_record and isinstance(self.run_record.errors, list):
                logs = list(self.run_record.errors)
            logs.append(str(e))
            self.update_run_status(RunStatus.FAILED, errors=logs)
            raise
