...
class S3StorageBackend(StorageBackend):
    ...
    def __init__(
        ...
        if aws_access_key_id and aws_secret_access_key:
            s3_kwargs.update({"aws_access_key_id": aws_access_key_id, "aws_secret_access_key": aws_secret_access_key})
            ...
            for page in paginator.paginate(**paginate_kwargs):
                ...
                if "CommonPrefixes" in page:
                    for prefix_info in page["CommonPrefixes"]:
                        dir_path = (
                            prefix_info["Prefix"][len(self.base_prefix) + 1 :]
                            if self.base_prefix
                            else prefix_info["Prefix"]
                        )
                        files.append(FileInfo(path=dir_path.rstrip("/"), size=0, is_directory=True, last_modified=None))
