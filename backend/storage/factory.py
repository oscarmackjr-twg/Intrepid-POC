...
def get_storage_backend(
    ...
    *,
    ...
    if storage_type == StorageType.S3:
        ...
        # New: allow explicit overrides via S3_INPUT / S3_OUTPUT / S3_OUTPUT_SHARED when set.
        if base_path is None:
            # Normalize any Windows-style backslashes from env to S3-style "/" keys
            def _norm(p: str | None) -> str | None:
                return p.replace("\\", "/").strip("/") if p else None

            s3_input = _norm(getattr(settings, "S3_INPUT", None))
            s3_output = _norm(getattr(settings, "S3_OUTPUT", None))
            s3_output_shared = _norm(getattr(settings, "S3_OUTPUT_SHARED", None))
            ...
            if area == "inputs":
                ...
                if s3_output_shared:
                    base_path = _join_prefix(settings.S3_BASE_PREFIX or "", s3_output_shared)
                else:
                    base_path = _join_prefix(settings.S3_BASE_PREFIX or "", area)
            else:
                base_path = _join_prefix(settings.S3_BASE_PREFIX or "", area)
